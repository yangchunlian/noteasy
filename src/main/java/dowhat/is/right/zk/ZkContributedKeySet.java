...
public class ZkContributedKeySet extends ZkSyncPrimitive {
  ...
  /**
   * Values that this instance failed to contribute to the distributed key set, for instance because
   * another instance had already contributed them (each entry in a contributed key set is "owned"
   ...
   * a contributed set, each user of the set owns the entries they have created. When such a user
   * disconnects from ZooKeeper, then all the entries they created will be automatically deleted.
   ...
   * @param myEntries  Entries that we wish to add to the set, while our ZooKeeper session is
   *                   active
   * @param allowDirty Whether the set should try to recover from a "dead" state and continue
   *                   showing "dirty" values while unsynchronized (session expired)
   */
  public ZkContributedKeySet(String path, String[] myContribution, boolean resurrectAfterExpiry) {
  ...
  /**
   * The complete set of entries created by the contributions of connected ZkContributedSet
   * instances
   *
  ...
  /**
   * A list of entries that we contributed, and which will be removed when our session to ZooKeeper
   ...
   *
   * @return The entries that this instance tried to contribute to the set, but which were
   * contributed by another ZkContributedSet instance
   */
  ...
  /**
   * Change your entry nodes. Note that the adjustment is not reflected immediately although any
   * listeners will only report the set has been updated when the contribution has been completely
   * changed (i.e. you do not get multiple update notifications while you are deleting previous
   * entries and adding new ones). If this method throws an exception because of a Zookeeper session
   * expiry, then when session is reestablished the set will try to reset its contribution to that
   * it was making before the method was called.
   *
   * @param myContribution A new list of entries you wish to contribute to the set
   * @throws KeeperException
   * @throws InterruptedException
   */
  public void adjustMyContribution(String[] myContribution)
    ...
    synchronized (this) {
      ...
      for (String entry : myObsoleteEntries) {
        zkClient().delete(rootPath + "/" + entry, -1);
        successfulContributions.remove(entry);
      }
      // Create new entries
      for (String entry : myNewEntries) {
        try {
          zkClient().create(rootPath + "/" + entry, new byte[0], ZooDefs.Ids.OPEN_ACL_UNSAFE,
